...
'''
将 Json 格式的标签转为 yolo 需要的 txt 格式标签
'''
# 获取某一路径下所有文件名
...
def preprocess(dataset_num, dataset_name, path="card_detection_data_final_edition",
    ...
    # 将 Json 转换成 yolo 格式 txt，转换后的标签存储路径
    ...
    for file_name in tqdm(files):
        ...
        # 获取图片名称。不同数据集字段名不同，有的 Json 文件存的是 image_name，有的存的是 filename
        ...
        w = image.shape[1]
        # 将原始图片写入到训练路径数据集
        cv.imwrite(os.path.join(res_images_path, image_name), image)

        """标签预处理
        根据 json 生成 txt
        类别（polyline）：'Image_Txt', 'Ad', 'Live', 'Video', 'Moment'
        ...
        for ori_label in temp:
            try:
                """按类别提取"""
                shape_type = ori_label['shape_attributes']['name']      # 如 rect、polyline
                property = ori_label['region_attributes']['propertyName']   # 如 标题、作者等
                if shape_type == 'polyline':
                    continue
                rect_flag = False
                # 处理前面的第 0-5 个类别，非 card 类标签：'标题', '图像', '作者', '评论', '时间', '详细内容'
                for label_i, label_type in enumerate(object_type[:6]): # 不执行
                    if property == label_type or shape_type == object_type_eng[label_i]:
                        label_w = ori_label['shape_attributes']["width"] / w
                        label_h = ori_label['shape_attributes']["height"] / h
                        label_x = ori_label['shape_attributes']["x"] / w + label_w / 2
                        label_y = ori_label['shape_attributes']["y"] / h + label_h / 2
                        labels_array.append([label_i, label_x, label_y, label_w, label_h])
                        rect_flag = True
                if rect_flag:
                    continue
                # 处理后面的第 6-10 个类别，card 类标签：图文、广告、视频、直播、动态
                for label_i, label_type in enumerate(object_type[6:]): # 执行
...
def preprocess_2(dataset_num, dataset_name, data_type_list):
    ...
    for file_name in tqdm(files):
        try:
            ...
            object_type_eng = ['Title', 'Image', 'Author', 'Comment', 'Time', 'Detail', 'Image_Text', 'Ad', 'Live', 'Video', 'Moment']
            ...
            for ori_label in temp:
                try:
                    ...
                    shape_type = ori_label['shape_attributes']['name']
                    property = ori_label['region_attributes']['propertyName']
                    if shape_type != 'rect' and shape_type != 'card':# 不是rect类别直接跳过
                        continue
                    for label_i,label_type in enumerate(object_type): # 只有前六个标签
                        if property == label_type:
                            ...
                            labels_array.append([label_i,label_x,label_y,label_w,label_h])
                            continue
                    for label_i,label_type in enumerate(object_type_eng): 
                        if property == label_type:
                            label_w = (ori_label['shape_attributes']["width"] / w * wn) / wf
                            label_h = (ori_label['shape_attributes']["height"] / h * hn) / hf
                            label_x = ((ori_label['shape_attributes']["x"] / w * wn) + wp_l) / wf + label_w / 2
...
if __name__ == "__main__":
    dataset_name = "jinritoutiao_feed_card_yolo"
